...
public class ScheduleContextTest {
  ...
  public void testEmptyStartTime() {
    ...
    schedule.setStart("");
    ScheduleContext sc = new ScheduleContext(schedule);
    if (print) {
  ...
  public void testGetEndTime() {
    ...
    DateTimeFormatter dtf = DateTimeFormatter.ofPattern(Schedule.DATETIME_FORMATS[0])
        .withZone(ZoneId.systemDefault());
    ZonedDateTime zdt = ZonedDateTime.parse(ScheduleData.TEST_TIME_2015, dtf);
    assertTrue("schedule context endtime is incorrect. Is " + sc1.getEndTime() + " should be "
               + zdt, zdt.equals(sc1.getEndTime()));
  }
  ...
  public void testFrequencyP1D() {
    Schedule schedule = ScheduleData.newTestInstance();
    schedule.setStart(ScheduleData.TEST_TIME_2015);
    schedule.setEnd("");
    schedule.setFrequency(TEST_FREQUENCY_1D);
    ScheduleContext sc = new ScheduleContext(schedule);
    ...
    assertTrue("next time does not match",
               sc.getNextTime().compareTo(zdt) == 0 && !sc.isComplete());
  }
  ...
  public void testFrequencyPT1H() {
    ...
    schedule.setStart(ScheduleData.TEST_TIME_2015);
    schedule.setEnd("");
    schedule.setFrequency(TEST_FREQUENCY_1H);
    ScheduleContext sc = new ScheduleContext(schedule);
    ZonedDateTime zdt = ZonedDateTime.now();
  ...
  public void testFrequencyPT1D1H() {
    ...
    schedule.setStart(ScheduleData.TEST_TIME_2015);
    schedule.setEnd("");
    schedule.setFrequency(TEST_FREQUENCY_1D1H);
    ScheduleContext sc = new ScheduleContext(schedule);
